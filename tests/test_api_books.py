...
async def _create_book(client, serial="123456", title="T", author="A"):
    return await client.post(
        "/books", json={"serial_number": serial, "title": title, "author": author}
    )
...
async def test_create_and_list_books(client):
    ...
    assert len(data) == 1
    assert {
        key for key in ["id", "serial_number", "title", "author", "is_borrowed"]
...
async def test_list_filters_and_pagination(client):
    ...
    response = await client.post(
        "/books/222222/borrow", json={"borrower_card": "333333"}
    )
    assert response.status_code == 200
    response = await client.get("/books")
...
async def test_borrow_and_return_happy_path(client):
    ...
    response = await client.post(
        "/books/123457/borrow", json={"borrower_card": "111111"}
    )
    assert response.status_code == 200
    book_data = response.json()
    assert (
        book_data["is_borrowed"] is True
        and book_data["borrowed_by"] == "111111"
        and book_data["borrowed_at"] is not None
    )
    response = await client.post("/books/123457/return")
    assert response.status_code == 200
    book_data = response.json()
    assert (
        book_data["is_borrowed"] is False
        and book_data["borrowed_by"] is None
        and book_data["borrowed_at"] is None
    )
...
async def test_borrow_already_borrowed(client):
    ...
    assert (
        await client.post("/books/123458/borrow", json={"borrower_card": "111111"})
    ).status_code == 200
    response = await client.post(
        "/books/123458/borrow", json={"borrower_card": "222222"}
    )
    assert response.status_code == 409
...
async def test_delete_borrowed_requires_flag(client):
    ...
    assert (
        await client.post("/books/123460/borrow", json={"borrower_card": "222222"})
    ).status_code == 200
...
async def test_set_status_borrow_and_free(client):
    ...
    response = await client.patch(
        "/books/123462/status", json={"is_borrowed": True, "borrower_card": "333333"}
    )
    assert response.status_code == 200
    book_data = response.json()
    assert (
        book_data["is_borrowed"] is True
        and book_data["borrowed_by"] == "333333"
        and book_data["borrowed_at"] is not None
    )
